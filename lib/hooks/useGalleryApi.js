...
function useGalleryApi() {
  ...
  const rate = useCallback(
    async ({ viewingImage, rating, feedback }) => {
      ...
      let { gallery: g1, success, message } = await response.json()

      if (success) {
        setUserImage(
          () => g1.images.filter((x) => x.username === localStorage.username)[0]
        )
        setGallery(() => g1)
